...
class SurveyModelTests(TestCase):
    ...
    def test_survey_slug_is_unique(self):
        Survey.objects.create(slug='s1', title_en='T', title_ko='T', type=Survey.LIKERT_5)
        with self.assertRaises(IntegrityError), transaction.atomic():
            Survey.objects.create(slug='s1', title_en='T2', title_ko='T2', type=Survey.LIKERT_5)

    def test_question_order_unique_per_survey(self):
        s = Survey.objects.create(slug='s2', title_en='T', title_ko='T', type=Survey.LIKERT_5)
    ...
    def test_option_order_unique_per_question(self):
        s = Survey.objects.create(slug='s3', title_en='T', title_ko='T', type=Survey.SINGLE_CHOICE)
    ...
    def test_daily_survey_date_unique(self):
        s = Survey.objects.create(slug='s4', title_en='T', title_ko='T', type=Survey.LIKERT_5)
    ...
    def test_response_unique_per_user_per_survey(self):
        s = Survey.objects.create(slug='s5', title_en='T', title_ko='T', type=Survey.LIKERT_5)
        SurveyResponse.objects.create(user=self.user, survey=s)
        with self.assertRaises(IntegrityError), transaction.atomic():
            SurveyResponse.objects.create(user=self.user, survey=s)

    def test_answer_unique_per_response_per_question(self):
        s = Survey.objects.create(slug='s6', title_en='T', title_ko='T', type=Survey.LIKERT_5)
        q = SurveyQuestion.objects.create(survey=s, order=1, prompt_en='Q', prompt_ko='Q')
