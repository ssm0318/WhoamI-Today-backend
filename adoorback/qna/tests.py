...
from qna.models import Article, Response, Question, Post, ResponseRequest
...
class PostAPITestCase(APITestCase):
    ...
    def test_feed_permissions(self):
        ...
        # test friend qna - self
        with self.login(username=current_user.username, password='password'):
            response = self.get('friend-qna-post-list')
...
class UserFeedTestCase(APITestCase):
    ...
    def test_user_feed(self):
        ...
        with self.login(username=current_user.username, password='password'):
            response = self.get(self.reverse('user-qna-post-list', pk=friend_user.id))
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.data['count'], 3)

        # user qna of non-friend
        with self.login(username=current_user.username, password='password'):
            response = self.get(self.reverse('user-qna-post-list', pk=1))
...
class ExceptionHandlerAPITestCase(APITestCase):
    ...
    def test_exception_raised(self):
        ...
        response = self.get('anonyous-qna-post-list')
