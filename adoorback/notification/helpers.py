...
from adoorback.utils.content_types import get_comment_type, get_like_type, get_response_type


def parse_message_ko(msg, actor_count):
    if actor_count > 2:
        print("actor_count > 2")
        pattern_c = re.compile(r"똑똑똑! (\w+)님, (\w+)님, 외 (\d+)명의 친구로부터 질문이 왔어요|(\w+)님, "
                               r"(\w+)님, 외 (\d+)명의 친구가 회원님의 (댓글을|답변을) 좋아합니다")
        match_c = pattern_c.search(msg)
        return {
            'user_a': match_c.group(1) or match_c.group(4),
            'user_b': match_c.group(2) or match_c.group(5)
        }

    if actor_count == 2:
        print("actor_count == 2")
        pattern_b = re.compile(r"똑똑똑! (\w+)님과 (\w+)님으로부터 질문이 왔어요|(\w+)님과 (\w+)님이 회원님의 (댓글을|답변을) 좋아합니다")
        match_b = pattern_b.search(msg)
        return {
            'user_a': match_b.group(1) or match_b.group(3),
            'user_b': match_b.group(2) or match_b.group(4)
        }

    if actor_count == 1:
        print("actor_count == 1")
        pattern_a = re.compile(r"똑똑똑! (\w+)님으로부터 질문이 왔어요|(\w+)님이 회원님의 (댓글을|답변을) 좋아합니다")
        match_a = pattern_a.search(msg)
        print("match_a: ", match_a)
        return {
            'user_a': match_a.group(1) or match_a.group(2),
            'user_b': None
        }
...
def parse_message_en(msg, actor_count):
    ...
    if actor_count == 2:
        pattern_b = re.compile(r"(Knock knock! |)(\w+) and (\w+) (have sent|liked your)")
        match_b = pattern_b.search(msg)
        return {
    ...
    if actor_count == 1:
        pattern_a = re.compile(r"(Knock knock! |)(\w+) (has sent|liked your)|(\w+) liked your")
        match_a = pattern_a.search(msg)
        return {
...
def find_like_noti(user, origin, target, noti_type):
...
def construct_message(noti_type, user_a_ko, user_b_ko, user_a_en, user_b_en, N, content_preview):
